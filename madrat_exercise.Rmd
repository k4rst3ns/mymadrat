...
We will create new download, read, and convert functions for WDI indicators Population, GDP, Employment rate, and Employment share in Agriculture, and agricultural GDP. Each indicator should be a separate parameter within one function.
...
Note that if direct download not possible, data files can be manually created in the inputdata/sources folder. In that case, a download function is not necessary.
...
The "fullVALIDATION" function combines all calcValid functions and produces a .mif file 
